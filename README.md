...
> ⚠️ **无编码器 ⇒ 无里程计**：直驱固件不做定距/定角闭环，`move` 的 `distance_cm` 与 `spin` 的 `angle_deg` 按**实测标定表的时长近似**到点自停（粗用，供微操/标定）；`arm` 的 `dist_cm` 按「每 cm ≈ 固定拍数」近似。原执行板上的速度 PID、里程计、到位判定随执行板一并裁撤。
